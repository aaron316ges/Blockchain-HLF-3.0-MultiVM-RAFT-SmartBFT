...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config ${ROOTPATH}/channel-artifacts/config_block.pb -o localhost:7050 --ordererTLSHostnameOverride orderer0.orgA-orderer.bft.com -c $CHANNEL --tls --cafile "$ORDERER_ORGA_CA"
  { set +x; } 2>/dev/null
