...
createAnchorPeerUpdate() {
  ...
  if [ $ORG = "peer0.orgA" ]; then
    HOST="peer0.orgA.bft.com"
    PORT=7051
  elif [ $ORG = "peer0.orgB" ]; then
    HOST="peer0.orgB.bft.com"
    PORT=8051
  elif [ $ORG = "peer0.orgC" ]; then
    HOST="peer0.orgC.bft.com"
    PORT=9051
  elif [ $ORG = "peer1.orgA" ]; then
    HOST="peer1.orgA.bft.com"
    PORT=10051
  elif [ $ORG = "peer1.orgB" ]; then
    HOST="peer1.orgB.bft.com"
    PORT=11051
  elif [ $ORG = "peer1.orgC" ]; then
    HOST="peer1.orgC.bft.com"
    PORT=12051
  else
    errorln "${ORG} unknown"
  fi
...
updateAnchorPeer() {
  peer channel update -o localhost:7050 --ordererTLSHostnameOverride orderer0.orgA-orderer.bft.com -c $CHANNEL_NAME -f ${ROOTPATH}/channel-artifacts/${CORE_PEER_LOCALMSPID}anchors.tx --tls --cafile "$ORDERER_ORGA_CA" >&log.txt
  res=$?
  cat log.txt
